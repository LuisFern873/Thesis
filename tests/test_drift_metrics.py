...
class TestComputeLayerDrift:
    ...
    def test_zero_drift_identical_states(self):
        ...
        assert drift["feature"]["raw"]  == pytest.approx(0.0, abs=1e-6)
        assert drift["feature"]["norm"] == pytest.approx(0.0, abs=1e-6)
        assert drift["norm"]["raw"]     == pytest.approx(0.0, abs=1e-6)
        assert drift["head"]["raw"]     == pytest.approx(0.0, abs=1e-6)
    ...
    def test_nonzero_drift_different_states(self):
        ...
        # diff = 1.0 per element, 4*4*3*3 = 144 elements
        # raw  = sqrt(144) = 12.0
        # norm = 12.0 / sqrt(144) = 1.0  (RMS drift = 1.0 per parameter)
        assert drift["feature"]["raw"]      == pytest.approx(12.0, rel=1e-4)
        assert drift["feature"]["norm"]     == pytest.approx(1.0,  rel=1e-4)
        assert drift["feature"]["n_params"] == 144
    ...
    def test_group_separation(self):
        ...
        # norm: 16 elements, diff=1 → raw=sqrt(16)=4.0, norm=4/sqrt(16)=1.0
        assert drift["norm"]["raw"]      == pytest.approx(4.0,          rel=1e-4)
        assert drift["norm"]["norm"]     == pytest.approx(1.0,          rel=1e-4)
        assert drift["norm"]["n_params"] == 16
        # feature: 16*3*3*3=432 elements → raw=sqrt(432), norm=sqrt(432)/sqrt(432)=1.0
        assert drift["feature"]["raw"]   == pytest.approx(np.sqrt(432), rel=1e-4)
        assert drift["feature"]["norm"]  == pytest.approx(1.0,          rel=1e-4)
        # head: 10*16=160 elements → raw=sqrt(160), norm=1.0
        assert drift["head"]["raw"]      == pytest.approx(np.sqrt(160), rel=1e-4)
        assert drift["head"]["norm"]     == pytest.approx(1.0,          rel=1e-4)
    ...
    def test_other_group_excluded(self):
        ...
        drift = compute_layer_drift(local, global_, taxonomy)
        assert all(v["raw"] == 0.0 for v in drift.values())
...
class TestAggregateDrift:
    ...
    def _make_drift(self, norm, feature, head):
        """Build a per-client drift dict in the new enriched format."""
        return {
            "norm":    {"raw": norm,    "norm": norm    / 1.0, "n_params": 1},
            "feature": {"raw": feature, "norm": feature / 1.0, "n_params": 1},
            "head":    {"raw": head,    "norm": head    / 1.0, "n_params": 1},
        }

    def test_mean_and_std(self):
        drifts = [
            self._make_drift(1.0, 2.0, 3.0),
            self._make_drift(3.0, 4.0, 5.0),
            self._make_drift(5.0, 6.0, 7.0),
        ]
        ...
        # norm_mean mirrors mean when n_params=1 (norm = raw / sqrt(1) = raw)
        assert result["norm"]["norm_mean"] == pytest.approx(3.0, rel=1e-5)

    def test_single_client(self):
        drifts = [self._make_drift(2.0, 4.0, 0.5)]
        result = aggregate_drift(drifts)
        assert result["norm"]["mean"]  == pytest.approx(2.0)
        assert result["norm"]["std"]   == pytest.approx(0.0)

    def test_all_zero(self):
        drifts = [self._make_drift(0.0, 0.0, 0.0)] * 5
        result = aggregate_drift(drifts)
        for g in ["norm", "feature", "head"]:
            assert result[g]["mean"]      == 0.0
            assert result[g]["std"]       == 0.0
            assert result[g]["norm_mean"] == 0.0
            assert result[g]["norm_std"]  == 0.0
